...
export default {
  async asyncData({ $axios }) {
    const ryoca = await $axios.$get(`https://raider.io/api/v1/characters/profile?region=eu&realm=twisting-nether&name=ryoca&fields=mythic_plus_best_runs%2Cmythic_plus_alternate_runs%2Cmythic_plus_scores_by_season%3Acurrent`)
    ...
    const enkadk = await $axios.$get(`https://raider.io/api/v1/characters/profile?region=eu&realm=draenor&name=enkadk&fields=mythic_plus_best_runs%2Cmythic_plus_alternate_runs%2Cmythic_plus_scores_by_season%3Acurrent`)
	  const characters = [ryoca, gaidin, pogbear, enkazin, enkamonk, enkadk]
